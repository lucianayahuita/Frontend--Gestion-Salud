  ...
  <div class="modal-overlay" @click.self="$emit('close')">
    <div v-if="formulario" class="modal-content">
      ...
      <div class="modal-header">
        <h2 class="modal-title">Registrar Nuevo Médico</h2>
      ...
      <form @submit.prevent="handleSubmit" class="registration-form">
        ...
        <div class="form-section">
          <span class="section-tag">Información Identificativa</span>
          <div class="grid-2">
        ...
        <div class="modal-footer">
          ...
          <button type="submit" class="btn-primary" :disabled="cargando">
            {{ cargando ? 'Registrando...' : 'Guardar Médico' }}
          </button>
        </div>
      </form>
    </div>
...
import { useRegistrarMedico } from '../composables/useRegistrarMedico';

const emit = defineEmits(['close', 'actualizar']);
const { formulario, cargando, errores, enviarRegistro } = useRegistrarMedico();
...
const handleSubmit = async () => {
  ...
  if (exito) {
    ...
    if (errores.value && typeof errores.value === 'object') {
      ...
      alert(errores.value || 'Ocurrió un error al procesar el alta del médico.');
    }
  }
};
</script>
