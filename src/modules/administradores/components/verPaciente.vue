<template>
  <div class="modal-overlay" @click.self="$emit('close')">
    <div class="modal-content">
      ...
      <div v-else-if="paciente && paciente.id === props.pacienteId" class="detalle">
...
import { Fingerprint, Calendar, Phone, MapPin } from 'lucide-vue-next'
...
watch(() => props.pacienteId, (newId) => {
  if (newId) {
    cargarPaciente(newId)
  }
})
</script>

<style scoped>
/* Estilos necesarios para que se vea profesional */
.modal-overlay {
...
.btn-close { border: none; background: #f3f4f6; cursor: pointer; border-radius: 50%; width: 30px; height: 30px; }
...
.paciente-nombre { font-size: 1.1rem; margin: 0; color: #111827; }
...
.campo-icon { color: #10b981; }
.campo-label { font-size: 0.85rem; color: #6b7280; width: 100px; }
...
.btn-secondary { padding: 8px 20px; border-radius: 8px; border: 1px solid #d1e2e1; cursor: pointer; }
