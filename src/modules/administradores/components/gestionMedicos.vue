<template>
  <div class="gp-page">
    ...
    <div class="gp-header">
      ...
      <button class="btn-registrar" @click="mostrarModal = true">Registrar Médico</button>
    </div>
    ...
    <div class="tabla-wrapper">
      <table class="tabla">
        ...
        <tbody>
          ...
          <tr v-else-if="medicosFiltrados.length === 0">
            <td colspan="5" class="estado-celda">No hay médicos registrados</td>
          </tr>
          <tr v-for="medico in medicosFiltrados" :key="medico.id" class="fila">
            <td class="td-id">{{ medico.id }}</td>
            <td class="td-nombre">{{ medico.name }}</td>
            <td>{{ medico.email }}</td>
            <td>
              <span class="badge" :class="medico.deleted_at === null ? 'badge-activo' : 'badge-inactivo'">
                {{ medico.deleted_at === null ? 'Activo' : 'Inactivo' }}
              </span>
            </td>
            <td class="td-acciones">
              <button class="btn-accion btn-ver" @click="abrirVer(medico)" title="Ver">
                <Eye :size="15" />
              </button>
              <button class="btn-accion btn-editar" @click="abrirEditar(medico)" title="Editar" >
                <Pencil :size="15" />
              </button>
              <button class="btn-accion btn-eliminar" @click="abrirEliminar(medico)" title="Eliminar">
                <Trash2 :size="15" />
              </button>
            </td>
          </tr>
    ...
    <RegistrarMedico
      ...
      @actualizar="manejarCierreYActualizar('registrar')"
    />

    <VerMedicos
      v-if="mostrarVer && medicoSeleccionado"
      :medicoId="medicoSeleccionado.id"
      @close="mostrarVer = false"
    />

    <UpdateMedicos
      v-if="mostrarEditar && medicoEditar"
      :medico="medicoEditar"
      @close="mostrarEditar = false"
      @actualizar="manejarCierreYActualizar('editar')"
    />

    <DeleteMedicos
      v-if="mostrarEliminar && medicoEliminar"
      :medico="medicoEliminar"
      @close="mostrarEliminar = false"
      @actualizar="manejarCierreYActualizar('eliminar')"
    />
...
// Importaciones estandarizadas a PascalCase
import RegistrarMedico from './RegistrarMedico.vue'
import VerMedicos from './verMedicos.vue'
import UpdateMedicos from './actualizarMedicos.vue'
//import DeleteMedicos from './DeleteMedicos.vue'
...
const medicoEditar = ref(null)
const mostrarEliminar = ref(false)
const medicoEliminar = ref(null)
...
const cargarMedicos = async () => {
  ...
  try {
    ...
    medicos.value = data.data.filter(u => u.rol_id === 2)
  } catch (e) {
    error.value = 'Error al cargar el listado de médicos'
  } finally {
...
const manejarCierreYActualizar = (tipo) => {
  if (tipo === 'registrar') mostrarModal.value = false
  if (tipo === 'editar') mostrarEditar.value = false
  if (tipo === 'eliminar') mostrarEliminar.value = false

  medicoEditar.value = null
  medicoEliminar.value = null
  setTimeout(() => {
    cargarMedicos()
  }, 60)
}

const abrirVer = (medico) => {
  medicoSeleccionado.value = medico
  mostrarVer.value = true
}

const abrirEditar = (medico) => {
  medicoEditar.value = medico
  mostrarEditar.value = true
}

const abrirEliminar = (medico) => {
  medicoEliminar.value = medico
