  ...
  <div class="modal-overlay" @click.self="$emit('close')">
    <div class="modal-content">
      ...
      <div class="modal-header">
        <h2 class="modal-title">Detalles del Farmacéutico</h2>
        <button class="btn-close" @click="$emit('close')">&times;</button>
      </div>

      <div v-if="cargando" class="estado-contenedor">
        <p class="animacion-carga">Consultando información en el servidor...</p>
      </div>
      ...
      <div v-else-if="farmaceutico" class="detalle-cuerpo">
        
        <div class="perfil-resumen">
          <div class="avatar-grande">
            {{ farmaceutico.name ? farmaceutico.name.charAt(0).toUpperCase() : 'F' }}
          </div>
          <h3>{{ farmaceutico.name }}</h3>
          <span class="badge-rol">Personal de Farmacia</span>
        </div>
        ...
        <div class="info-grid">
          ...
          <div class="info-item">
            ...
            <div class="info-valor-caja">
              <span class="indicador-estado" :class="farmaceutico.deleted_at === null ? 'activo' : 'inactivo'"></span>
              <span>{{ farmaceutico.deleted_at === null ? 'Activo en Farmacia' : 'Inactivo / De baja' }}</span>
            </div>
          </div>
        </div>
      ...
      <div class="modal-footer">
        <button type="button" class="btn-secondary" @click="$emit('close')">Cerrar Detalle</button>
      </div>
...
import { useVerFarmaceutico } from '../composables/useVerFarmaceutico';
...
const { farmaceutico, cargando, error, cargarDetalle } = useVerFarmaceutico();

onMounted(() => {
  if (props.farmaceuticoId) {
    cargarDetalle(props.farmaceuticoId);
  }
});
</script>
...
.avatar-grande {
  ...
  background: #f3f0fb; color: #6d28d9; font-size: 24px;
  font-weight: 700; display: flex; align-items: center; justify-content: center;
  margin-bottom: 12px; box-shadow: 0 4px 10px rgba(109, 40, 217, 0.15);
}
.perfil-resumen h3 { margin: 0 0 6px 0; font-size: 1.15rem; color: #1a2b2e; font-weight: 600; }
.badge-rol { font-size: 11px; background: #f3f0fb; color: #6d28d9; font-weight: 700; padding: 4px 12px; border-radius: 12px; text-transform: uppercase; }
