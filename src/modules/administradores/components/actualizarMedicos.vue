  ...
  <div class="modal-overlay" @click.self="$emit('close')">
    <div class="modal-content">
      ...
      <div class="modal-header">
        <h2 class="modal-title">Modificar Cuenta de Médico</h2>
      ...
      <form @submit.prevent="handleSubmit" class="update-form">
        ...
        <div class="form-section">
          <span class="section-tag">Información del Facultativo</span>
          <div class="field-icon disabled-field">
            <User :size="16" class="icon" />
            <input :value="medico?.name" disabled class="input-disabled" />
          </div>
          <p class="field-help">El nombre completo se mantendrá intacto en el servidor.</p>
        </div>

        <div class="form-section">
          <span class="section-tag">Dirección de Contacto</span>
          <div class="field-icon">
            <Mail :size="16" class="icon" />
            <input 
              v-model="email" 
              type="email" 
              placeholder="medico.nuevo@gmail.com" 
              required 
            />
          </div>
        </div>
        ...
        <div class="modal-footer">
          ...
          <button type="submit" class="btn-primary" :disabled="cargando">
            {{ cargando ? 'Guardando...' : 'Actualizar Correo' }}
          </button>
        </div>
...
import { useActualizarMedico } from '../composables/useActualizarMedico';
...
const { email, cargando, errores, ejecutarActualizacion } = useActualizarMedico(props.medico);

const handleSubmit = async () => {
  if (cargando.value) return; 
  
  const exito = await ejecutarActualizacion(props.medico.id);
  if (exito) {
    emit('actualizar');
  } else {
    if (errores.value && typeof errores.value === 'object') {
      const listaErrores = Object.values(errores.value).flat().join('\n');
      alert("Error de validación en los datos:\n\n" + listaErrores);
    }
  }
};
</script>
